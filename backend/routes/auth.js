    ...
    try {
        ...
        res.json({
            ...
            user: {
                ...
                theme: 'dark',
                sidebar_order: user.sidebar_order || null
            }
        });
    } catch (error) {
...
router.post('/logout', authMiddleware, async (req, res) => {
    try {
        await logActivity(req.user.id, 'LOGOUT', `Benutzer ${req.user.username} hat sich abgemeldet`);
...
// Update user theme (kept for backward compat, but always forces dark)
router.post('/update-theme', authMiddleware, async (req, res) => {
    try {
        await pool.query(
            "UPDATE users SET theme = 'dark' WHERE id = ?",
            [req.user.id]
        );
...
// Update user preferences (sidebar_order etc)
router.post('/update-preferences', authMiddleware, async (req, res) => {
    try {
        const { sidebar_order } = req.body;

        // Ensure column exists (auto-migration)
        try {
            await pool.query(
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS sidebar_order TEXT DEFAULT NULL"
            );
        } catch (e) { /* Column may already exist */ }

        const updates = {};
        if (sidebar_order !== undefined) {
            updates.sidebar_order = JSON.stringify(sidebar_order);
        }
