...
router.get('/', authMiddleware, requirePermission('lendings.view'), async (req, res) => {
    try {
       ...
       LEFT JOIN accessories acc ON l.accessory_id = acc.id
...
router.post('/', authMiddleware, requirePermission('lendings.create'), async (req, res) => {
    try {
        ...
        if (!asset_id && !container_id && !accessory_id) {
            return res.status(400).json({ error: 'Entweder Asset, Container oder Zubehör muss ausgewählt werden' });
        }

        // Check if asset/container/accessory is available
        if (asset_id) {
        ...
        if (accessory_id) {
            // Check for existence and status
            const [accs] = await pool.query('SELECT status FROM accessories WHERE id = ?', [accessory_id]);
            if (accs.length === 0) return res.status(404).json({ error: 'Zubehör nicht gefunden' });
            if (accs[0].status === 'defekt') return res.status(400).json({ error: 'Defektes Zubehör kann nicht verliehen werden' });

            // Check for active lending
            const [active] = await pool.query('SELECT id FROM lendings WHERE accessory_id = ? AND returned = FALSE', [accessory_id]);
            if (active.length > 0) return res.status(400).json({ error: 'Dieses Zubehör ist bereits verliehen' });
        }

        // Sanitize IDs
        const sanitizedAssetId = asset_id === '' ? null : (asset_id || null);
        const sanitizedContainerId = container_id === '' ? null : (container_id || null);
        const sanitizedAccessoryId = accessory_id === '' ? null : (accessory_id || null);

        // Insert lending
        const [result] = await pool.query(
            `INSERT INTO lendings 
       (asset_id, container_id, accessory_id, user_id, borrower_name, borrower_type, start_date, planned_end_date, notes) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [sanitizedAssetId, sanitizedContainerId, sanitizedAccessoryId, req.user.id, borrower_name, borrower_type, start_date, planned_end_date, notes]
        );
