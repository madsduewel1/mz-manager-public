...
router.get('/', authMiddleware, requirePermission('errors.view'), async (req, res) => {
    ...
    `;
        ...
        if (search) {
            query += ' AND (er.description LIKE ? OR er.reporter_name LIKE ? OR a.inventory_number LIKE ? OR er.category LIKE ?)';
...
// PUBLIC: Get assets in a room hierarchy (Room -> Containers -> Assets) (NO AUTH REQUIRED)
router.get('/public/assets/room/:roomId', async (req, res) => {
    try {
        const [assets] = await pool.query(
            `SELECT a.id, a.inventory_number, a.type, a.model 
             FROM assets a 
             JOIN containers c ON a.container_id = c.id 
             WHERE c.id = ? OR c.parent_container_id = ? 
             ORDER BY a.inventory_number`,
            [req.params.roomId, req.params.roomId]
        );
        res.json(assets);
    } catch (error) {
        console.error('Get public room assets error:', error);
        res.status(500).json({ error: 'Serverfehler' });
    }
});
...
router.post('/public', upload.single('photo'), async (req, res) => {
    try {
        const { qr_code, description, reporter_name, reporter_email, selected_asset_id, container_id: body_container_id, category } = req.body;
       ...
       (asset_id, container_id, description, photo_path, reporter_name, reporter_email, status, priority, category) 
       VALUES (?, ?, ?, ?, ?, ?, 'offen', 'mittel', ?)`,
            [asset_id, container_id, description, photo_path, reporter_name, reporter_email, category]
        );
