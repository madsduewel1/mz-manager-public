...
router.post('/device-models', authMiddleware, requirePermission('models.create'), async (req, res) => {
    try {
        ...
        await logActivity(req.user.id, 'MODEL_CREATE', `Modell ${manufacturer} ${model_name} angelegt`);
...
router.delete('/device-models/:id', authMiddleware, requirePermission('models.delete'), async (req, res) => {
    try {
        ...
        await logActivity(req.user.id, 'MODEL_DELETE', `Modell ID ${req.params.id} gelöscht`);
...
router.post('/rooms', authMiddleware, requirePermission('rooms.create'), async (req, res) => {
    try {
        ...
        await logActivity(req.user.id, 'ROOM_CREATE', `Raum ${name} angelegt`);
...
router.delete('/rooms/:id', authMiddleware, requirePermission('rooms.delete'), async (req, res) => {
    try {
        ...
        await logActivity(req.user.id, 'ROOM_DELETE', `Raum ID ${req.params.id} gelöscht`);
...
router.post('/roles', authMiddleware, requirePermission('roles.manage'), async (req, res) => {
    ...
    try {
        ...
        await logActivity(req.user.id, 'ROLE_CREATE', `Rolle ${name.trim()} angelegt`);
...
router.put('/roles/:id', authMiddleware, requirePermission('roles.manage'), async (req, res) => {
    ...
    try {
        ...
        await connection.commit();
        await logActivity(req.user.id, 'ROLE_UPDATE', `Rolle ${name.trim()} bearbeitet`);
...
router.delete('/roles/:id', authMiddleware, requirePermission('roles.manage'), async (req, res) => {
    ...
    try {
        ...
        await connection.commit();
        await logActivity(req.user.id, 'ROLE_DELETE', `Rolle ID ${id} gelöscht`);
...
// Batch create device models
router.post('/device-models/batch', authMiddleware, requirePermission('models.create'), async (req, res) => {
    const connection = await pool.getConnection();
    try {
        const { models } = req.body;

        if (!Array.isArray(models) || models.length === 0) {
            return res.status(400).json({ error: 'Keine Modelle zum Importieren gefunden' });
        }
        ...
        for (const modelData of models) {
            try {
                const { type, manufacturer, model_name, description } = modelData;

                if (!type || !manufacturer || !model_name) {
                    throw new Error('Typ, Hersteller und Modellname sind erforderlich');
                }

                await connection.query(
                    'INSERT INTO device_models (type, manufacturer, model_name, description) VALUES (?, ?, ?, ?)',
                    [type, manufacturer, model_name, description || null]
                );
                ...
                results.failed++;
                results.errors.push({
                    item: modelData.model_name || 'Unbekannt',
                    error: err.message
                });
            }
        }
        ...
        await connection.commit();
        await logActivity(req.user.id, 'MODEL_IMPORT', `${results.success} Modelle importiert`);
        res.json(results);
    } catch (error) {
        if (connection) await connection.rollback();
        console.error('Batch create models error:', error);
...
// Batch create rooms
router.post('/rooms/batch', authMiddleware, requirePermission('rooms.create'), async (req, res) => {
    const connection = await pool.getConnection();
    try {
        const { rooms } = req.body;

        if (!Array.isArray(rooms) || rooms.length === 0) {
            return res.status(400).json({ error: 'Keine Räume zum Importieren gefunden' });
        }
        ...
        for (const roomData of rooms) {
            try {
                const { name, building, floor, capacity } = roomData;
                ...
                const [existing] = await connection.query(
                    'SELECT id FROM containers WHERE name = ? AND type = "raum"',
                    [name]
                );

                if (existing.length > 0) {
                    throw new Error(`Raum "${name}" existiert bereits`);
                }
                ...
                await connection.query(
                    "INSERT INTO containers (name, type, building, floor, qr_code, capacity) VALUES (?, 'raum', ?, ?, ?, ?)",
                    [name, building || null, floor || null, qr_code, capacity || 0]
                );
        ...
        await connection.commit();
        await logActivity(req.user.id, 'ROOM_IMPORT', `${results.success} Räume importiert`);
        res.json(results);
    } catch (error) {
        if (connection) await connection.rollback();
